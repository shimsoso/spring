		...
		<div>
			<table class="table table-sm table-striped">
				<thead class="table-dark">
					<tr>
						<th class="col">매장 아이디</th>
						<th class="col">테이블번호</th>
				...
				<tbody>
					<c:if test="${list.size() < 1}">
						<tr>
							<td colspan="5">등록 된 테이블이 없습니다</td>					
						</tr>
					</c:if>
					<c:forEach var="item" items="${list}">
						<tr>
							...
							<td>${item.title}</td>
							<td>${item.person}</td>
							<td><a href="delete/${item.tableId}" class="btn btn-outline-danger btn-sm">삭제</a> <a href="update/${item.tableId}" class="btn btn-outline-info btn-sm">변경</a></td>							
						</tr>
					</c:forEach>					
				</tbody>
				<tfoot>
